...
const HomePage = () => {
  ...
  const [radio, setRadio] = useState(null); // Changed to null for better clarity
  ...
  // Trigger filtering when checked or radio change
  useEffect(() => {
    if (checked.length || radio !== null) {
      filterProduct();
    } else {
      getAllProducts(); // Fetch all products if no filter is selected
    }
  }, [checked, radio]);
